AJS.test.require("jira.webresources:select-pickers",function() {
    ...
    function(
        ...
        test("it should properly fetch new results when scroll treshold is met",function() {
            var $element = this.selectBuilder(20).appendTo(jQuery('#qunit-fixture')),
                select = new ScrollableSingleSelect({
                    element: $element,
                    pageSize: 5,
        ...
        test('it should display error message when failed to fetch next page',function() {
            var $element = this.selectBuilder(2).appendTo(jQuery('#qunit-fixture')),
                select = new ScrollableSingleSelect({
                    element: $element,
                    pageSize: 2
                });
