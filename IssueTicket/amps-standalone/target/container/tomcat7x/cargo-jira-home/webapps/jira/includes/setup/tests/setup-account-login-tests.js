AJS.test.require("jira.webresources:jira-setup", function(){
    ...
    var View = require("jira/setup/setup-account-login-view");

    JIRA.Templates.Setup.Account.formSwitchNonExistingAccount = function(data, output){
        output.append("<span class=\"jira-setup-account-email-not-existing hidden\">");
        output.append("<a href=\"#\" id=\"jira-setup-account-form-switch-nonexisting\" class=\"jira-setup-account-form-switch\">");
        output.append("</span>");

        return output.toString();
    };

    JIRA.Templates.Setup.Account.formSwitchAfterSubmitNonExistingAccount = function(data, output){
        output.append("<span class=\"jira-setup-account-email-not-existing-after-submit hidden\">");
        output.append("<a href=\"#\" id=\"jira-setup-account-form-switch-nonexisting-after-submit\" class=\"jira-setup-account-form-switch\">");
        output.append("</span>");
    ...
    var markup = JIRA.Templates.Setup.layoutContent({
        content: JIRA.Templates.Setup.Account.loginForm({})
    });
    ...
    var loginAuiMsgSelector = ".jira-setup-account-cannot-login-message";
    var recheckAccountAuiMsgSelector = ".jira-setup-account-cannot-recheck-account-message";

    module("JIRA Setup Account Login page", {
        _validEmails: [],
        setup: function(){
            ...
            $("#qunit-fixture").append(
                    $("<div></div>")
                            .addClass("jira-account-login-form")
                            .html(markup)
            );

            this.setupTracker = {};
            this.setupTracker.sendUserLoggedInEvent = this.sandbox.spy();
        },
    ...
    test("should add analytics event when user is logged in", function(){
        // when
        this.initializeView();
        ...
        var defer = $.Deferred();
        this.macUtilMock.loginUser = this.sandbox.stub().returns(defer.promise());

        this.testObj.onSubmitClicked(e);
        defer.resolve();

        // then
        ok(this.setupTracker.sendUserLoggedInEvent.calledOnce, "Analytics event should be raised")
    });

    test("should not send analytics event when login form is invalid", function(){
        // when
        this.initializeView();
        this.populateFieldsOverridingDefaults({password: ""});
        ...
        // then
        ok(!this.setupTracker.sendUserLoggedInEvent.called, "Analytics event should be skipped")
    });

    test("when email field is invalid form submit shouldn't call any MAC method", function(){
        // when
        this.macUtilMock.loginUser = this.sandbox.stub().returns($.Deferred().promise());
        this.macUtilMock.checkIfUserExists = this.sandbox.stub().returns($.Deferred().promise());

        this.initializeView();
        this.populateFieldsOverridingDefaultsAndSubmitForm({email: INVALID_EMAIL, password: VALID_EMAIL});

        //then
        sinon.assert.notCalled(this.macUtilMock.loginUser);
    ...
    test("should show form switch proposition for unregistered email after email field blur", function(){
        // when
        var mailCheckDefer = $.Deferred();
        this.macUtilMock.checkIfUserExists = this.sandbox.stub().returns(mailCheckDefer.promise());
        ...
        ok(!this.testObj.ui.descEmailNotExisting.hasClass("hidden"), "switch form message for after blur should be visible");
        ok(this.testObj.ui.descEmailNotExistingAfterSubmit.hasClass("hidden"), "switch form message for after submit should be hidden");
    });

    test("should request registration form when form switch proposition link, shown after email field blur, is clicked", function(){
        // when
        var mailCheckDefer = $.Deferred();
        this.macUtilMock.checkIfUserExists = this.sandbox.stub().returns(mailCheckDefer.promise());
        ...
        this.testObj.on("registration-form-requested", switchHandler);
        this.testObj.ui.descEmailNotExisting.find("a").click();

        // then
        sinon.assert.calledOnce(switchHandler, "registration form was requested");
    });
    ...
    test("should hide form switch proposition, shown after email field blur, when email changed to existing one", function(){
        ...
        this.macUtilMock.checkIfUserExists = this.sandbox.stub().returns(mailCheckDefer.promise());
        this.triggerEmailCheck(VALID_EMAIL2);
        ...
        ok(this.testObj.ui.descEmailNotExisting.hasClass("hidden"), "switch form message for after blur should be hidden");
        ok(this.testObj.ui.descEmailNotExistingAfterSubmit.hasClass("hidden"), "switch form message for after submit should be hidden");
    });

    test("should show form switch proposition for unregistered email after unsuccessful login attempt", function(){
        // when
        var loginDefer = $.Deferred();

        this.macUtilMock.loginUser = this.sandbox.stub().returns(loginDefer);
        this.macUtilMock.isEmailNotVerifiedErrorMessage = this.sandbox.stub().returns(true);

        this.initializeView();
        this.populateFieldsOverridingDefaultsAndSubmitForm();
        loginDefer.reject({error: "awesome error"});

        // then
        ok(this.testObj.ui.descEmailNotExisting.hasClass("hidden"), "switch form message for after blur should be hidden");
        ok(!this.testObj.ui.descEmailNotExistingAfterSubmit.hasClass("hidden"), "switch form message for after submit should be visible");
    });

    test("should request registration form when form switch proposition link, shown after unsuccessful login attempt, is clicked", function(){
        // when
        var loginDefer = $.Deferred();
    ...
    test("should hide form switch proposition, shown after unsuccessful login attempt, when email changed to existing one", function(){
        // when
        var loginDefer = $.Deferred();
        var mailCheckDefer = $.Deferred();
        ...
        this.macUtilMock.isEmailNotVerifiedErrorMessage = this.sandbox.stub().returns(false);
        this.submitForm();
    ...
    test("should keep form disabled when logging in user", function(){
        // when
        var loginDefer = $.Deferred();
        this.macUtilMock.loginUser = this.sandbox.stub().returns(loginDefer.promise());
    ...
    test("should keep form disabled when generating license", function(){
        ...
        ok(this.testObj.ui.spinnerLogging.hasClass("hidden"), "login spinner should disappear" );
    ...
    test("should require to check customer agreement checkbox before proceeding with login", function(){
        // when
        this.initializeView();
        this.populateFieldsOverridingDefaults();
    ...
    test("should display a message if unable to login user", function(){
        // when
        var loginDefer = $.Deferred();
        this.macUtilMock.loginUser = this.sandbox.stub().returns(loginDefer.promise());
        ...
        //then
        equal($(loginAuiMsgSelector, this.testObj.$el).text(),
            "setup.account.logging.problem.titlesetup.account.logging.problem.content", "login failure message displayed");
    });

    test("message about inability to login should be rendered as an error since the third time", function(){
        // when
        var loginDefer = $.Deferred();
        this.macUtilMock.loginUser = this.sandbox.stub().returns(loginDefer.promise());
        ...
        // then
        equal($(loginAuiMsgSelector).hasClass("aui-message-warning"), true, "message is rendered as a warning");

        // when
        this.submitForm();

        // then
        equal($(loginAuiMsgSelector).hasClass("aui-message-warning"), true, "message shown for the second time is rendered as a warning");

        // when
        this.submitForm();

        // then
        equal($(loginAuiMsgSelector).hasClass("aui-message-error"), true, "message shown for the third time is rendered as an error");
    });
    ...
    test("there should be only one global message at a time", function(){
        ...
        // then
        equal($(loginAuiMsgSelector).length, 1, "login failure message displayed");
        ...
        // then
        equal($(loginAuiMsgSelector).length, 0, "no login failure message");
    ...
    test("after successful login should raise an event requesting license generation and show appropriate spinner", function(){
        ...
        this.macUtilMock.loginUser = this.sandbox.stub().returns(loginDeferred.promise());
        ...
        sinon.assert.calledWith(handler, "login");
    });
});