...
public class IssueCRUD extends HttpServlet{
    ...
    private ChangeHistoryManager test;
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) 
		...
		if ("y".equals(req.getParameter("new"))) {
			...
			for (Issue iss : issues)
				  //ch.add(mng.getChangeHistoryById(iss.getId()))
	...
	private List<Issue> getIssues(HttpServletRequest req) {
		...
		com.atlassian.query.Query query = jqlClauseBuilder.assigneeIsCurrentUser().buildQuery();
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if ("y".equals(req.getParameter("edit"))) {
			...
			Project project = projectService.getProjectByKey(user,"MON").getProject();
			//ServiceOutcome<List<Project>> lists =  projectService.getAllProjects(user);
			// resp.setContentType("text/html");
			// List<Project> list= lists.get();
			 
			 //while (i<list.size())
		     //resp.getWriter().write("<html><body>"+list.get(0).toString()+"====="+list.get(1).toString());
